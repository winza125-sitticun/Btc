...
from btc_core.ai.providers.base import (
    AIProviderError,
...
class OpenAICompatibleProviderClient:
    ...
    async def __aenter__(self) -> "OpenAICompatibleProviderClient":
        return self
    ...
    def _request_payload(self, snapshot: AIAnalysisSnapshot) -> dict[str, Any]:
        return {
            ...
            "messages": [
                ...
                {
                    "role": "user",
                    "content": "Analyze this bounded market snapshot and return JSON:\n"
    ...
    def _parse_response(self, response: httpx.Response) -> AIDecision:
        try:
            ...
            raise AIProviderError("AI provider returned invalid JSON envelope", code="INVALID_JSON") from exc

        try:
            content = envelope["choices"][0]["message"]["content"]
        except (KeyError, IndexError, TypeError) as exc:
            raise AIProviderError("AI provider response did not contain message content", code="INVALID_SCHEMA") from exc
        if not isinstance(content, str) or not content.strip():
            raise AIProviderError("AI provider response content was empty", code="INVALID_SCHEMA")

        try:
            data = json.loads(content)
        except (TypeError, ValueError) as exc:
            raise AIProviderError("AI provider returned invalid JSON content", code="INVALID_JSON") from exc
        if not isinstance(data, dict):
            raise AIProviderError("AI provider JSON content must be an object", code="INVALID_SCHEMA")
        ...
        try:
            ...
            raise AIProviderError("AI provider decision failed schema validation", code="INVALID_SCHEMA") from exc
