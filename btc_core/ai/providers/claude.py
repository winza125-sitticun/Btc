...
class ClaudeProviderClient:
    ...
    async def __aenter__(self) -> "ClaudeProviderClient":
        return self
    ...
    def _request_payload(self, snapshot: AIAnalysisSnapshot) -> dict[str, Any]:
        return {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "Analyze this bounded market snapshot:\n" + snapshot.model_dump_json(),
                }
            ],
            "output_config": {
                "format": {
    ...
    def _parse_response(self, response: httpx.Response) -> AIDecision:
        ...
        try:
            ...
            raise AIProviderError("Claude returned invalid JSON content", code="INVALID_JSON") from exc
        if not isinstance(data, dict):
            raise AIProviderError("Claude JSON content must be an object", code="INVALID_SCHEMA")
        data = dict(data)
        ...
        try:
            ...
            raise AIProviderError("Claude decision failed schema validation", code="INVALID_SCHEMA") from exc
