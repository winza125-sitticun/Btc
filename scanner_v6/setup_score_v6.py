...
def _market_score(side: str, btc_bias: str, tradingview_price: float, provider_price: float, max_basis_diff_pct: float) -> Optional[int]:
...
def score_setup(*, side: str, r4: str, r1: str, r15: str, adx: Any, di_plus: Any, di_minus: Any, rr_tp1: Any, rr_tp2: Any = None, rr_tp3: Any = None, btc_bias: str, tradingview_price: Any, provider_price: Any, funding: Any, rsi: Any, max_basis_diff_pct: float = 1.0, max_long_funding: float = 0.03, min_short_funding: float = -0.04) -> Dict[str, Any]:
    side = str(side).upper()
    critical = {
        "adx": _as_float(adx), "di_plus": _as_float(di_plus), "di_minus": _as_float(di_minus),
        "rr_tp1": _as_float(rr_tp1), "tradingview_price": _as_float(tradingview_price),
        "provider_price": _as_float(provider_price), "funding": _as_float(funding), "rsi": _as_float(rsi),
    }
    ...
    if trend is None: return _invalid("TREND_MISALIGNMENT")
    strength = _strength_score(side, critical["adx"], critical["di_plus"], critical["di_minus"])
    if strength is None: return _invalid("STRENGTH_INVALID")
    ...
    if structure is None: return _invalid("STRUCTURE_INVALID")
    market = _market_score(side, str(btc_bias).upper(), critical["tradingview_price"], critical["provider_price"], float(max_basis_diff_pct))
    if market is None: return _invalid("PRICE_BASIS_MISMATCH")
    funding_points = _funding_score(side, critical["funding"], float(max_long_funding), float(min_short_funding))
    if funding_points is None: return _invalid("FUNDING_INVALID")
    timing = _timing_score(side, critical["rsi"])
    if timing is None: return _invalid("TIMING_INVALID")
    components = {"trend": trend, "strength": strength, "structure": structure, "market": market, "funding": funding_points, "timing": timing}
    total = int(sum(components.values()))
    return {"valid": True, "score": max(0, min(total, 100)), "reason": "OK", "components": components}
...
def score_passes_threshold(result: Optional[Dict[str, Any]], threshold: int) -> bool:
    if not isinstance(result, dict) or result.get("valid") is not True:
        return False
    try:
        score = int(result.get("score")); threshold_value = int(threshold)
    except (TypeError, ValueError):
        return False
