...
def candidate(direction=Direction.LONG, existing=()):
    return OutcomeEvaluationCandidate(
        ...
        model="gemini-test", symbol="BTCUSDT", direction=direction,
...
def bar(timeframe, close_time, *, high, low, close):
    ...
    return MarketOutcomeBar(
        symbol="BTCUSDT", timeframe=timeframe, open_time=close_time-duration,
        close_time=close_time, high=high, low=low, close=close,
    )


def mfe_bars(hours, *, high=110.0, low=95.0):
    return tuple(bar("15m", START+timedelta(minutes=15*i), high=high, low=low, close=100) for i in range(1, hours*4+1))
...
def test_exact_horizon_close_and_long_signed_excursions():
    result = evaluate_outcome(
        ...
        ), mfe_bars=mfe_bars(1), now=START+timedelta(hours=2),
    )
    assert result.matured_at == START+timedelta(hours=1)
    assert result.horizon_price == 102
    assert result.signed_return_pct == 2.0 and result.directional_hit is True
    assert result.mfe_pct == 10.0 and result.mae_pct == -5.0
    assert result.data_quality == "FULL" and result.state is OutcomeState.EVALUATED
...
def test_short_return_flips_and_missing_data_fails_closed():
    short = evaluate_outcome(
        ...
        mfe_bars=mfe_bars(1, high=105, low=90), now=START+timedelta(hours=2),
    )
    assert short.signed_return_pct == 10.0
    assert short.mfe_pct == 10.0 and short.mae_pct == -5.0
    invalid = evaluate_outcome(candidate(), EvaluationHorizon.H1, horizon_bars=(), mfe_bars=mfe_bars(1), now=START+timedelta(hours=2))
    assert invalid.state is OutcomeState.INVALID_DATA and invalid.data_quality == "MISSING"
    ...
    assert partial.state is OutcomeState.EVALUATED and partial.data_quality == "PARTIAL"


def evidence(i, *, matured_at, hit=True, signed=2.0, horizon="1H", quality="FULL", direction=Direction.LONG):
    return PerformanceEvidence(
        multi_agent_run_id=f"run-{i}", agent_attempt_id=i+1, role=AgentRole.TECHNICAL,
        provider=AIProvider.GEMINI, model="gemini-test", symbol="BTCUSDT",
        direction=direction, market_regime=MarketRegime.BULL, horizon=horizon,
        state=OutcomeState.EVALUATED, data_quality=quality, matured_at=matured_at,
        directional_hit=hit, signed_return_pct=signed,
    )
...
def test_historical_weight_is_cold_start_and_anti_lookahead():
    ...
    summary = summarize_performance(rows, as_of=as_of, role=AgentRole.TECHNICAL, provider=AIProvider.GEMINI, model="gemini-test", horizon=EvaluationHorizon.H1)
    assert summary.sample_count == 29 and summary.multiplier == 1.0
...
def test_locked_multiplier_formula_after_30_full_canonical_rows():
    ...
    summary = summarize_performance(rows, as_of=as_of, role=AgentRole.TECHNICAL, provider=AIProvider.GEMINI, model="gemini-test", horizon=EvaluationHorizon.H1)
    assert summary.sample_count == 30 and summary.hit_rate == 1.0
    assert summary.mean_signed_return_pct == 2.0 and summary.normalized_expectancy == 1.0
    assert summary.quality_score == 1.0 and summary.multiplier == 1.25
...
async def test_historical_weight_persists_metric_inputs_before_return():
    ...
    multiplier = await MultiAgentPerformanceService(repo).historical_weight_for(AgentRole.TECHNICAL, AIProvider.GEMINI, "gemini-test", as_of)
    assert multiplier == 1.25 and len(repo.snapshots) == 1
    assert repo.snapshots[0].sample_count == 30 and repo.snapshots[0].horizon == "1H"
